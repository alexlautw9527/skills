...
1. **Story**
2. **Responsibility**（In Scope / Out of Scope）
3. **Applicable Constraints & Invariants**
4. **UI Source**
5. **UI Spec**
6. **Existing Code**

功能規格沒有提供對應的 UI Source 或 UI Spec 時，省略整個欄位，不寫「無」或其他替代文字。Responsibility 可依後文規則整欄省略，省略時同樣不寫「無」。
...
所有 Slice 放在同一份計畫中。Slice 的排列順序就是實作順序，Story 覆蓋使用 Markdown 連結指向同一份文件內的 Slice heading。
...
- `index.md` 保留範圍與依據、Current State、Constraints & Invariants、Shared Concerns、Slice 順序與相對連結，以及 Story 覆蓋。
- 每個 Slice 檔案只包含該 Slice 的欄位。Current State、Constraints & Invariants 與 Shared Concerns 不複製到 Slice 檔案。
...
Current State 是實作位置與責任邊界的地圖，只記錄目前已存在，而且會影響實作選擇的內容。
...
- 預計新增、修改或刪除的檔案。
- 已能從 Slice 的 Story 或 Existing Code 得知的重複敘述。

### 4. 整理 Constraints & Invariants
...
每項內容直接說明規則及其對實作選擇的影響。只有能由功能規格、目前程式碼或使用者指示支持的內容才能列入。generated code、選單同步、型別產生等做法，寫「執行時從專案指示文件查證」，並在 Existing Code 指向目前查到的入口，不寫死路徑。

### 5. 整理 Shared Concerns

當同一套規格或實作分工會出現在兩個以上 Slice 時，在 Constraints & Invariants 之後新增 **Shared Concerns** 一節。規則全文留在 Constraints & Invariants；本節只寫適用 Slice、規格來源，以及程式應抽共用或由各頁自行處理的部分。
...
沒有跨 Slice 共用項時，省略整節 Shared Concerns，不寫 `None`。只影響單一 Slice 的規則留在該 Slice 的 Responsibility。
...
Constraints & Invariants 與 Shared Concerns 的分工：

- **Invariants** 只寫操作完成後一定要成立的規則。
- **Shared Concerns** 只寫多 Slice 如何共用同一套規格理解與實作邊界。
- 若某規則只影響單一 Slice，留在該 Slice 的 Responsibility，不進 Shared Concerns。

各 Slice 若適用 Shared Concerns，在 **Applicable Constraints & Invariants** 末尾加一行 `Shared Concerns：<標題>`，只列 heading 文字，不重述五個欄位。
...
5. 同一可觀察行為若有兩套仍在使用的實作，放進同一個 Slice。Current State 與 Existing Code 兩邊都要標。
6. 同一 Story 若含兩個可獨立交付、可獨立觀察的狀態，拆成兩個 Slice。Story 覆蓋要把該 Story 連到所有相關 Slice。
7. 外部介面尚未定案或由其他文件維護時，仍以可觀察行為切分，不建立只處理 API、schema、types、hooks、元件或測試的技術 Slice。
8. 逐項核對 story 清單。每個 story 都要指向至少一個 Slice；明確排除的項目則在 Story 覆蓋中標出 Non-Goals 或排除理由。
...
Story 與 Existing Code 已能看出邊界時，可省略整欄。省略時不寫「無」。
...
Existing Code 告訴實作者應先閱讀哪些現有程式碼，以及這些位置在目前行為中負責什麼。它不是預計變更的檔案清單，也不需要完整盤點所有相關檔案。
...
3. 文字修訂不能改變功能規格與目前程式碼支持的事實、路徑、symbol、Slice 順序或 Story 覆蓋關係。
4. 修訂後重新執行完成檢查。
...
模板中的 UI Source、UI Spec、Responsibility、觸發條件只示範有對應內容時的格式。沒有對應來源或可省略的欄位，必須省略整個欄位。Slice 2 示範在邊界已清楚時省略 Responsibility。
...
- 單一 Slice 的既有做法已放入該 Slice 的 Existing Code；強制執行方式與 generated code 限制已放入 Constraints。
...
- 只收錄兩個以上 Slice 共用的項目；單 Slice 規則留在該 Slice 的 Responsibility。
...
- 各 Slice 的 Applicable Constraints & Invariants 只引用 Shared Concerns 標題，沒有重述全文。

### Slices

- Slice 依可觀察行為切分，沒有按 UI、state、API、package、types、hooks、元件或測試分成技術階段。
- 同一 Story 若含兩個可獨立交付的可觀察狀態，已拆成不同 Slice，並在 Story 覆蓋列出全部連結。
- 兩套仍在使用的實作已放進同一個 Slice，Existing Code 兩邊都有標。
- Slice 依實作順序連續編號，標題、檔名與連結中的編號一致。
- 每個 Slice 的欄位順序為 Story、Responsibility、Applicable Constraints & Invariants、UI Source、UI Spec、Existing Code；可省略的欄位已整欄省略。
- 出現相鄰 Slice 同畫面、Story 被拆、或 UI spec 含排除畫面時，已撰寫 Responsibility。
- 每個 Story 都使用穩定 ID 並連到功能規格的對應 heading。
- 只有已確認的設計來源與 UI spec 才會出現；缺少來源時已省略整個欄位。
- Existing Code 的頂層先說明現有位置的責任，下一層才列查證過的 path、route 或 symbol。
- Existing Code 沒有變更狀態、完整未來檔案樹、props、未查證的介面細節，或對需求內容的重述。
...
- 只有使用者明確要求時才拆檔。
- Current State 與 Constraints & Invariants、Shared Concerns 只出現在 `index.md`。
- 每個 Slice 檔案只包含該 Slice 的欄位。
- Slice 檔名描述可觀察行為，編號、順序與 `index.md` 連結一致。

### 表達與泛化

- 交付前已依 `make-ai-readable-zh` skill 修訂，且修訂沒有改變技術事實、Slice 順序與 Story 覆蓋。
