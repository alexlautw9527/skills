...
- 使用者指定或目前環境可確認的功能規格，例如需求文件、issue、acceptance criteria、設計稿、API 規格或架構決策。
...
4. 找出每項需求的規格依據。設計稿、介面規格、資料模型或架構決策只有在已確認適用時才能寫入計畫。
...
- `Requirements` 使用既有的穩定需求 ID 與來源連結。沒有穩定 ID 時，使用需求標題與來源連結。
- `Scope` 說明完成後可觀察到的結果、沿用的既有資料流，以及刻意不處理的相鄰能力。
- `Constraints and Invariants` 只引用已在共用章節完整說明的規則。
- `Map` 只列開始實作前需要閱讀且已查證的現有位置，不是變更檔案清單。
- `Verification` 直接證明呼叫端取得 Slice 宣稱的結果。可使用畫面操作、API request / response、CLI command、integration test、contract test、acceptance test，或可觀察的資料與外部系統結果。
...
- [<stable-requirement-id-or-title>](requirement-link)
...
- 每個 Slice 的 Verification 能直接證明所宣稱結果，不以內部 class、資料表、元件或 mock 存在作為完整交付證據。
